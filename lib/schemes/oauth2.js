...
export default class Oauth2Scheme {
  ...
  login() {
    const opts = Object.assign(
      ...
      this.options.custom_login_params
    );
  ...
  async _handleCallback(uri) {
    ...
    // idToken
    let idToken = parsedQuery[this.options.token_key];
    ...
    if (this.options.token_type) {
      ...
      idToken = this.options.token_type + " " + idToken;
    }

    // Store token
    this.$auth.setToken(this.name, accessToken);
    this.$auth.setIdToken(this.name, idToken);
